...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            valid_bbox = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading)
            
            ## step 2 : loop over all detected objects
            for detection in detections:
                
                ## step 3 : extract the four corners of the current detection
                _, _x, _y, _z, _, _w, _l, _yaw = detection
                det_bbox = tools.compute_box_corners(_x, _y, _w, _l, _yaw)
                ...
                det_polygon = Polygon(det_bbox)
                valid_polygon = Polygon(valid_bbox)
                iou = det_polygon.intersection(valid_polygon).area/det_polygon.union(valid_polygon).area
    ...
    all_positives = len(labels_valid[labels_valid == True])
